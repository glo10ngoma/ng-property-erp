...
export function EmployeesPage() {
  ...
  async function saveEmployee(form: FormData, current?: Employee | null) {
    ...
    if (current?.id) {
      ...
      employees.reload();
      contracts.reload();
      navigate(`/personnel/employees/${response.data.id}`);
    }
  }
...
function HrCatalogModal({ title, label, item, onClose, onSubmit }: { title: string; label: string; item?: HrService | HrPosition | null; onClose: () => void; onSubmit: (form: FormData) => void | Promise<void> }) {
...
function EmployeeModal({ title, employee, services, positions, onClose, onSubmit }: { title: string; employee?: Employee | null; services?: HrService[]; positions?: HrPosition[]; onClose: () => void; onSubmit: (form: FormData) => void | Promise<void> }) {
...
function employeePayload(form: FormData) {
  return {
    ...
    contract_start_date: optionalStringValue(form, 'contract_start_date'),
    contract_end_date: optionalStringValue(form, 'contract_end_date'),
    contract_currency: optionalStringValue(form, 'contract_currency') ?? 'USD',
    contract_status: optionalStringValue(form, 'contract_status'),
    contract_observations: optionalStringValue(form, 'contract_observations'),
  };
}
