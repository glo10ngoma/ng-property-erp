...
export function StockMovementsPage() {
  ...
  const filtered = useMemo(() => movements.data.filter((movement) => includesText(movement, query) &&
    ...
    (!itemId || String(movement.item_code) === itemId) &&
    (!category || movement.category === category) &&
    (!start || movement.movement_date >= start) &&
    (!end || movement.movement_date <= end)),
  ...
  return (
    <section>
      ...
      <div className="maintenance-filter-bar stock-filter-bar">
        ...
        <input type="date" value={start} onChange={(e) => setStart(e.target.value)} title="Date début" />
        <input type="date" value={end} onChange={(e) => setEnd(e.target.value)} title="Date fin" />
      ...
      <div className="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>Type</th><th>Article</th><th className="right">Entrée</th><th className="right">Sortie</th><th>Unité</th><th className="right">Coût unitaire</th><th className="right">Valeur</th><th className="right">Stock après</th><th>Référence</th><th>Source</th><th>Utilisateur</th><th>Observation</th></tr></thead>
          <tbody>
            {filtered.map((item) => {
              const bucket = movementType(item);
              const quantity = Number(item.quantity ?? 0);
              const isEntry = bucket === 'ENTRY' || (bucket === 'INVENTORY_ADJUSTMENT' && quantity >= 0);
              return (
                <tr key={item.id} className="clickable-row" onClick={() => navigate(`/stock/movements/${item.id}`)}>
                  <td>{shortDate(item.movement_date)}</td>
                  <td>{movementLabel(item)}</td>
                  <td>{item.item_name}</td>
                  <td className="right">{isEntry ? quantity : '—'}</td>
                  <td className="right">{!isEntry ? quantity : '—'}</td>
                  <td>{item.unit ?? items.data.find((stock) => stock.code === item.item_code)?.unit ?? '—'}</td>
                  <td className="right">{money(item.unit_price ?? 0)}</td>
                  <td className="right">{money(quantity * Number(item.unit_price ?? 0))}</td>
                  <td className="right">{item.quantity_after ?? '—'}</td>
                  <td>{item.document_number ?? item.reference ?? '—'}</td>
                  <td>{item.source ?? '—'}</td>
        ...
        {!filtered.length && <EmptyState message="Aucun mouvement trouvé." />}
      </div>
    </section>
  );
}
