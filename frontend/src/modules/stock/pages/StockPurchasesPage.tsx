import { Eye, FileSpreadsheet, Plus, RotateCcw, WalletCards } from 'lucide-react';
...
export function PurchaseCreateModal({ items, onClose, onSubmit }: { items: StockItem[]; onClose: () => void; onSubmit: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  const itemOptions = items.map((item) => ({ value: item.id, label: item.name, meta: `${item.code ?? '-'} | ${item.store ?? 'Magasin'}` }));
  ...
  async function submit(form: FormData) {
    setError('');
    const paymentType = String(form.get('payment_type') ?? 'DEFERRED');
    const payload = {
      ...
      initial_payment_amount: Number(form.get('initial_payment_amount') ?? 0),
    ...
    if (!payload.supplier_name.trim()) {
      setError('Renseignez le fournisseur.');
      return;
    }
    if (payload.lines.some((line) => !line.stock_item_id || line.quantity <= 0)) {
      setError('Chaque ligne doit contenir un article et une quantite valide.');
      return;
    }
    ...
    <form className="stock-purchase-modal" onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }}>
      <div className="modal-section">
        ...
        <div className="form-grid stock-purchase-grid">
          ...
          <label>Mode paiement<select name="payment_method" defaultValue="BANK"><option value="CASH">Especes</option><option value="BANK">Banque</option><option value="MOBILE_MONEY">Mobile Money</option><option value="OTHER">Autre</option></select></label>
          <label>Paiement<select name="payment_type" defaultValue="DEFERRED"><option value="CASH">Comptant</option><option value="PARTIAL">Partiel</option><option value="DEFERRED">Differe</option></select></label>
          <label>Montant paye initial<input name="initial_payment_amount" type="number" min="0" step="0.01" defaultValue="0" /></label>
          <label>Date echeance<input name="due_date" type="date" /></label>
        ...
        <div className="table-wrap stock-document-lines-wrap">
          <table className="stock-document-lines">
            ...
            <tbody>{lines.map((line) => <tr key={line.rowId}>
              ...
              <td className="actions actions-compact"><button type="button" className="icon-btn danger" title="Supprimer" onClick={() => setLines((current) => current.length > 1 ? current.filter((entry) => entry.rowId !== line.rowId) : current)}>×</button></td>
            </tr>)}</tbody>
          </table>
        </div>
...
export function PurchaseReceiveModal({ purchase, onClose, onSubmit }: { purchase: StockPurchaseDetail; onClose: () => void; onSubmit: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  async function submit(form: FormData) {
    ...
    if (!lines.length) {
      setError('Aucune ligne de reception saisie.');
      return;
    }
    if (lines.some((line, index) => Number(line.quantity_received) > rows[index].remaining)) {
      setError('Une quantite recue depasse le reste a recevoir.');
      return;
    }
    await onSubmit({
  ...
  return <Modal title={`Recevoir marchandises - ${purchase.purchase_number}`} onClose={onClose}>
    <form className="stock-purchase-modal" onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }}>
      <div className="modal-section">
        <h3>Reception</h3>
        <div className="form-grid stock-purchase-grid">
          <label className="locked-field">N° reception<input value="Automatique (BR-000001)" readOnly /></label>
