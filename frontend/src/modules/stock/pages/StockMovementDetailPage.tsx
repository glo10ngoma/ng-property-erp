...
export function StockMovementDetailPage() {
  ...
  const exportExcel = () => exportXlsxWorkbook(`Mouvement_${movement.movement_number ?? movement.id}.xlsx`, [
    { name: 'Mouvement', rows: [{
      numero: movement.movement_number, type: movementLabel(movement), document_origine: movement.document_number ?? '—',
      date: shortDate(movement.movement_date), utilisateur: movement.user_name ?? '—', article: movement.item_name,
      magasin: movement.store ?? '—', quantite: movement.quantity, unite: movement.unit ?? '—',
      cout_unitaire: movement.unit_price ?? 0, valeur: value, stock_avant: movement.quantity_before ?? 0,
      stock_apres: movement.quantity_after ?? 0, reference: movement.document_reference ?? movement.reference ?? '—',
      motif: movement.document_reason ?? '—', observations: movement.document_observations ?? movement.notes ?? '—',
      piece_jointe: movement.attachment_file_name ?? 'Non disponible',
    }] },
    { name: 'Historique', rows: movement.history.map((row) => ({
      date: shortDate(row.created_at), action: row.action, description: row.description ?? '—', utilisateur: row.user_name ?? '—',
    })) },
  ]);

  return <section className="stock-movement-detail">
    ...
    <div className="detail-section"><h4>Informations générales</h4><div className="detail-list">
      <span>Article</span><strong>{movement.item_code} - {movement.item_name}</strong>
      ...
      <span>Pièce jointe</span><strong>{movement.attachment_file_name
        ? movement.attachment_file_url
          ? <a href={movement.attachment_file_url} target="_blank" rel="noreferrer">{movement.attachment_file_name}</a>
          : movement.attachment_file_name
        : 'Aucune pièce jointe'}</strong>
    </div></div>
