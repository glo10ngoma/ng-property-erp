...
export function TenantCredits() {
  ...
  async function sendCreditEmail(payload: { recipient: string; cc: string; subject: string; message: string }) {
    ...
    try {
      ...
      setEmailOpen(false);
      setSuccess('Reçu de crédit locataire envoyé par email avec succès.');
    } catch (error: any) {
      setEmailError(String(error?.response?.data?.message ?? error?.message ?? 'Impossible d’envoyer le reçu du crédit.'));
    } finally {
