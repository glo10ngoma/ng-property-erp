    ...
    <section>
      ...
      <form className="form-grid" onSubmit={(event) => { event.preventDefault(); save(new FormData(event.currentTarget)); }}>
        <div className="detail-section">
          ...
          <label>Immeuble<select value={buildingId} onChange={(event) => { setBuildingId(event.target.value); setUnitId(''); }} required><option value="">Selectionner un immeuble</option>{buildings.data.map((building) => <option key={building.id} value={building.id}>{building.name}</option>)}</select></label>
          <label>Unite / Appartement<select name="unit_id" value={unitId || selectedUnit?.id || ''} onChange={(event) => setUnitId(event.target.value)} required>{availableUnits.map((unit) => <option key={unit.id} value={unit.id}>{unit.building_name} / {unit.number} - {statusLabel(unit.status)} - {money(unit.monthly_rent)}</option>)}</select></label>
          <label>Locataire<select name="tenant_id" required><option value="">Selectionner un locataire</option>{tenants.data.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.first_name} {tenant.last_name}</option>)}</select></label>
        </div>
        ...
        <div className="detail-section">
          ...
          <label>Duree<input name="duration" placeholder="Ex. 12 mois" /></label>
        ...
        <div className="detail-section">
          ...
          <label>Piece jointe<input type="file" accept="application/pdf,image/*" onChange={(event) => setContractName(event.target.files?.[0]?.name ?? '')} /></label>
          <label>Nom du fichier<input name="contract_file_name" value={contractName} onChange={(event) => setContractName(event.target.value)} placeholder="contrat-bail.pdf" /></label>
        ...
        <div className="detail-section">
          <h4>Observations</h4>
          <label>Notes<textarea name="notes" placeholder="Observations internes" /></label>
        </div>
